...
| **记忆预取** | `MemoryManager.prefetch_all()` 加载历史记忆上下文 |
...
while self.turn_count < self.max_turns:
    ...
    # 3. 流式调用 LLM
    response = self._stream_llm_call(kwargs)
    ...
    for tool_call in normalized.tool_calls:
        ...
        # 将工具结果追加到消息历史
        self.messages.append(format_tool_result(tool_call, result))
