...
class TestSendOrEditMediaStripping:
    ...
    @pytest.mark.asyncio
    async def test_first_send_passes_reply_to_message_id(self):
        """Initial send replies to the source platform message when configured."""
        adapter = MagicMock()
        send_result = SimpleNamespace(success=True, message_id="msg_1")
        adapter.send = AsyncMock(return_value=send_result)
        adapter.MAX_MESSAGE_LENGTH = 4096
        ...
        consumer = GatewayStreamConsumer(
            adapter,
            "chat_123",
            reply_to_message_id="source_msg_123",
        )
        await consumer._send_or_edit("Hello")

        adapter.send.assert_called_once()
        assert adapter.send.call_args[1]["reply_to"] == "source_msg_123"
